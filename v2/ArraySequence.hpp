...
public:
    ...
    ArraySequence<T>* ArrayMap(std::function<T(T)> f) {
        return dynamic_cast<ArraySequence<T>*>(Instance()->Map(f));
    }

    ArraySequence<T>* ArrayWhere(std::function<bool(T)> predicate) {
        return dynamic_cast<ArraySequence<T>*>(Instance()->Where(predicate));
    }
