...
public:
    ...
    ListSequence<T>* ListMap(std::function<T(T)> f) {
        return dynamic_cast<ListSequence<T>*>(Instance()->Map(f));
    }

    ListSequence<T>* ListWhere(std::function<bool(T)> predicate) {
        return dynamic_cast<ListSequence<T>*>(Instance()->Where(predicate));
    }
